"""
Multi-seed value function plotter for the Gaussian–Gaussian mixture agent.
Imports env_1d and sarsa_agent from time_space_Gaussian_Guassian.py,
trains N seeds, and then makes a plot of all seeds superimposed 
each seed is at high transparency, then we compute an average curve
across seeds and plot that in bold to show the general trend.

Place this file in the same directory as time_space_Gaussian_Gaussian.py.
"""
...
_mod = importlib.import_module("time_space_Gaussian_Gaussian")
env_1d = _mod.env_1d
...
# CONFIGURATION
# ============================================================
N_SEEDS = 50          # Number of random seeds to train
...
# Plot resolution (number of x-points)
PLOT_RESOLUTION = 200

# Whether to include the closeness reward curve in the plot
...
def train_single_seed(seed):
    ...
    agent = sarsa_agent(n_centers=N_CENTERS, sigma=SIGMA,
                        agent_std=AGENT_STD,
                        lr=LR, gamma=GAMMA,
                        start_range=(-ENV_SIZE, ENV_SIZE))
...
def main():
    ...
    if SEED_LIST is not None:
        ...
        seeds = rng.randint(0, 100_000_000, size=N_SEEDS).tolist()
    ...
    # Bold average curves
    ax.plot(xs, avg_L_low,  color='red',  linewidth=2.5,
            label=f'Low-σ Left (σ={AGENT_STD[0]})')
    ax.plot(xs, avg_L_high, color='red',  linewidth=2.5, linestyle='--',
            label=f'High-σ Left (σ={AGENT_STD[1]})')
    ax.plot(xs, avg_R_low,  color='blue', linewidth=2.5,
            label=f'Low-σ Right (σ={AGENT_STD[0]})')
    ax.plot(xs, avg_R_high, color='blue', linewidth=2.5, linestyle='--',
            label=f'High-σ Right (σ={AGENT_STD[1]})')
    ...
    # # Closeness reward (only if enabled)
    # if HAVE_CLOSENESS_REWARD:
    #     closeness = results[0]["closeness"].copy()
    #     # --- START NORMALISE CLOSENESS (comment out with the block above) ---
    #     if NORMALISE_CURVES:
    #         c_min, c_max = closeness.min(), closeness.max()
    #         if c_max - c_min > 1e-12:
    #             closeness = (closeness - c_min) / (c_max - c_min)
    #     # --- END NORMALISE CLOSENESS ----------------------------------------
    #     ax.plot(xs, closeness, color='green', linewidth=2, label='Closeness Reward')

    ax.axvline(0, color='black', linestyle='--', linewidth=0.7)
    ax.set_title(f"Learned Value Functions : $\sigma_1$ = {AGENT_STD[0]:.2f} vs $\sigma_2$ = {AGENT_STD[1]:.2f})",
                 fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(f"ts_gg_gaussian{AGENT_STD[0]}_gaussian{AGENT_STD[1]}.png", dpi=150)
    plt.show()
    print(f"\nSaved to ts_gg_gaussian{AGENT_STD[0]}_gaussian{AGENT_STD[1]}.png")
