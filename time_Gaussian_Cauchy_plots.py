"""
Multi-seed mode probability plotter for the time-only Gaussian-Cauchy agent.
Imports env_1d and sarsa_agent from time_Gaussian_Cauchy.py,
trains N seeds, records mode probability evolution per episode,
and plots the mean mode probability across all seeds.

Place this file in the same directory as time_Gaussian_Cauchy.py.
"""
...
import os
_mod = importlib.import_module("time_Gaussian_Cauchy")
env_1d = _mod.env_1d
...
# CONFIGURATION
# ============================================================
N_SEEDS = 1000         # Number of random seeds to train
...
def train_single_seed(seed):
    ...
    for ep in range(EPISODES):
        ...
        while not terminated and steps < STEPS_PER_EPISODE:
            ...
            agent.policy_update(state, direction_id, mode_id, reward,
                                next_state, next_direction_id, next_mode_id, terminated)
...
def main():
    ...
    ax.plot(episodes_axis, avg_pi[:, 0], color='red', linewidth=2,
            label=f'$\sigma$ = {AGENT_STD[0]}',alpha=0.6)
    ax.plot(episodes_axis, avg_pi[:, 1], color='blue', linewidth=2,
            label=f'$\gamma$ = {AGENT_STD[1]}',alpha=0.6)
    ...
    ax.set_title(f"Mixture Weight Evolution for $\sigma$={AGENT_STD[0]} vs $\gamma$={AGENT_STD[1]}", fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(f"t_gc_mode_probs_gaussian{AGENT_STD[0]}_cauchy{AGENT_STD[1]}.png", dpi=150)
    plt.show()
    print(f"\nSaved to t_gc_mode_probs_gaussian{AGENT_STD[0]}_cauchy{AGENT_STD[1]}.png")

     #Plot 5: Lin reg of each mode probability vs number of episodes
    # filename = f"linreg_gaussian{AGENT_STD[0]}_cauchy{AGENT_STD[1]}"
    ...
    # ax1.plot(episodes_axis, linreg1, linewidth=1, color="red", label=f"$\ln(\pi_1)={m1:.5f}t+{b1:.5f}$")
    # ax1.scatter(episodes_axis, log_pi1, color="green", s=5)
    ...
    # plt.tight_layout()
    # plt.savefig(f"{filename}/t_gg_pi_gaussian{AGENT_STD[0]}_cauchy{AGENT_STD[1]}.png")
    # plt.show()
